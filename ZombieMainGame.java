...
public class ZombieMainGame {
    ...
    public void startGame() {
        ...
        traps = new ZombieTraps(player, zombies, zombieLevel, graphics, this);
    ...
    public void nextLevel() {
        if (levelNum == maxLevel) {
            ...
            traps = new ZombieTraps(player, zombies, zombieLevel,
                        graphics, this);
            score += 1000;
            initialScore = score;
            trapTimer = -1;
    ...
    /**
     * Kills the passed zombie.
     *
     * @param zombie Zombie to kill and remove from list.
     */
    public void killZombie(Zombie zombie) {
        // TODO: burning, death animation
        zombies.remove(zombie);       
    }
     
    /**
     * A helper method for trap pickups.
     */
    public void pickupTrapHelper() {
        // TrapTimer gets set negative each frame if action isn't held down.
        // If it's negative, we set it to 5 seconds before decrementing.
        if (trapTimer < 0) {
            trapTimer = FPS * 5;
        }
        if (player.isOnTrap() && input.get("action")) {
            if (--trapTimer == 0) {
                player.grabFireTrap();
                // TODO: update level to remove trap
            }
        }
    }
    ...
    private void playingHelper() {
        ...
        if (input.get("action")) {
            if (player != null && player.getPosition() != null) {
                if (player.isOnTrap()) {
                    // If the player is standing on a trap when he presses
                    // action, he has to stand still while he picks it up.
                    leftRight = 0;
                    upDown = 0;
                    run = false;
                    pickupTrapHelper();
                } else {
                    traps.dropTrap(player.getPosition().x,
